{******************************************************************************}
{*              Delphi example using the Flyweight pattern                    *}
{*                                                                            *}
{* The Flyweight design pattern is used when we need to create a lot of       *}
{* objects of a class. Since every object consumes memory space that can be   *}
{* crucial for low memory devices, such as mobile devices or embedded         *}
...
begin
  ...
  try
    try
      ...
      var LDocument: String := 'AAZZBBZB';
      var LPointSize: Integer := 10;
      var LCharacterFactory := TCharacterFactory.Create;
      try
        for var LChar: Char in LDocument do
        begin
          Inc(LPointSize);
          LCharacterFactory.GetCharacter(LChar).Display(LPointSize);
        end;
      finally
        LCharacterFactory.Free;
      end;
    except
      on E: Exception do
